...
void test_BIT(void)
{
    ...
    for (uint8_t i = 0x40; i <= 0x7f; i++) {
        ...
        if (nibble != 0x6 && nibble != 0xe) {
            printf("\tBit check instructions | opcode 0x%x\n", i);
            BIT_tester(i);
        } else {
            printf("\tBit check instructions | opcode 0x%x (HL)\n", i);
            BIT_HL_tester(i);
        }
    }
    printf("\t...");
...
int main()
{
    ...
    CU_Suite *test_suite = CU_add_suite("Bit check instructions unit tests", NULL, NULL);
    if (test_suite == NULL) {
    ...
    if (CU_add_test(test_suite, "", test_BIT) == NULL) {
        printf("Failed to add test to Bit check instructions unit test suite\n");
        CU_cleanup_registry();
        return CU_get_error();
    }
